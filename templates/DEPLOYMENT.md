...
## 0. Quick Start — the whole platform in 10 minutes

**Goal:** a live CBT website + database + accounts, on free tiers only.

1. **Create the database.** Sign up at [supabase.com](https://supabase.com) → *New project* → choose a region near you → set a database password and SAVE it.
2. **Install the schema.** In Supabase open **SQL Editor** → *New query* → paste the ENTIRE contents of `database/complete-schema.sql` → **Run**. It is idempotent — re-running never loses data. This creates every table, RPC function, Row-Level-Security policy and the storage bucket in one shot.
3. **Turn on email auth.** Supabase → *Authentication → Providers*: Email already enabled. For same-day classroom use also open *Authentication → Sign In / Up → Allow new users to sign up* (you can disable public sign-up later and invite from *Authentication → Users → Add user*).
4. **Get your two keys.** Supabase → *Project Settings → API*: copy the **Project URL** and the **anon public** key (never the `service_role` key).
5. **Put the keys in the site.** Open `assets/js/app.js` (or your site's config) and set `SB_URL` / `SB_KEY` — in the delivered build these are already set for the HMG project. For a NEW project, update the two values in `student.html`, `teacher.html`, `admin.html` and the other pages that carry them (search for `supabase.co`).
6. **Publish the site.** Push all files to a GitHub repo, then:
   - **Vercel/Netlify:** *Import project* → framework "Other" → deploy (no build command, output dir = repo root), **or**
   - **GitHub Pages:** repo *Settings → Pages → Deploy from branch → root*.
   - **Cloudflare Pages:** *Create project → Connect to Git* → framework "None".
7. **Create your first accounts.** Open the live site → *Teacher Hub → Sign up* (first teacher), and *Admin Panel* (admin sign-up). Students don't create accounts — they enter an **exam code**.
8. **Publish your first exam.** Teacher Hub → *Build Assessment* → paste/upload a question CSV (any School Connect / GOSA / HMG format) → set duration + pass mark → **Publish** → give students the 6-character code.
9. **Verify.** Open `student.html` on your phone → enter the code → take the exam → see the score in Teacher Hub → *Results*. Also run `deployment_validator.html` on the live site for a self-check.

Stuck? Jump to §10 (post-deployment test plan) or §11 (troubleshooting). Everything below expands each step in detail.
...
## 15. Phase 12G — updating an existing deployment & healing published papers

Phase 12G adds the **publish-time data doctor**: every import and save path now stores the canonical question contract (assertion–reason stems, case-study passages, 5-option shift healed, items as arrays, letter keys). The student-side healing from Phases 12D–12F remains as a second line of defence.

### 15.1 Update the deployed site

1. Upload/replace the full contents of `cbt-system-PHASE12G.zip` to your hosting (all files, preserving folders) — or push to the GitHub repo that auto-deploys.
2. The service-worker cache version bumps to `hmg-cbt-shell-v12-phase12g-v1`; every visitor's shell refreshes on their **next** reload after the deploy (hard refresh Ctrl+F5 forces it immediately).
3. Verify you are on the new build: open the live `sw.js` in the browser — the first `CACHE_NAME` must read `…phase12g-v1`.

### 15.2 Heal already-published papers (one-time)

The doctor runs at SAVE time, so papers published before 12G keep their old rows until re-saved. For every affected exam, either:

- **Teacher Hub → My Assessments → Edit → Add Questions → re-upload the original CSV** → Save (the merge save rewrites the whole set through the doctor), **or**
- **Teacher Hub → My Assessments → Question Bank (📚) → Save All Edits** — even with no edits made, the save re-stores every question in canonical form.

New imports and new publishes are canonical automatically — no action needed for papers created after the update.

### 15.3 If a student still sees something odd after all this

1. Have them **hard-refresh** (Ctrl+F5) or close ALL tabs of the site and reopen — the old cached shell serves for exactly one navigation after an update.
2. Check the exam in the Teacher Hub **Review** view — if it looks correct there but wrong for the student, the student is on a cached build (step 1 fixes it).
3. If it looks wrong in Review too, re-import the CSV per §15.2 — the database rows predate the doctor.

---

### 15.5 Phase 12H — image questions (image_mcq / hotspot / evidence_mcq)

Figures vanished for papers re-saved during the 12G window (items were array-wrapped; the image renderers expect objects). 12H fixes all three layers — doctor, student heal and renderers. After uploading the PHASE12H build:

- **Rendering is fixed immediately** — the student app now heals damaged rows at load; no re-import is required for students to see the figures again.
- **(Recommended)** re-save affected papers once (Question Bank → Save All Edits) so the database stores the canonical object form as well.
- **Hotspot questions imported before this fix lost their answer key at import** (the letter-stripper destroyed region-label text like "Central vacuole") — re-upload those CSVs once (Edit → Add Questions) so the free-text key is stored.
