...
(function (w, d) {
  ...
  var CBTTypes = {
    ...
    /* Runtime questions from the platform's own CSV parser carry the key as
       `.ans` (mcq letters, "A,C", numbers, exact short-answer text); the
       grader's internal contract uses `.answer` (School Connect shape).
       This adapter makes BOTH shapes grade identically — without it every
       runtime question looked "keyless" and scored 0/unmarkable. */
    _adaptKey: function (q) {
      if (!q || q.answer !== undefined || q.ans === undefined) return q;
      var nq = {};
      for (var k in q) { if (Object.prototype.hasOwnProperty.call(q, k)) nq[k] = q[k]; }
      nq.answer = q.ans;
      return nq;
    },
    ...
    grade: function (q, given) {
      ...
      if (t === 'multi_select') {
        /* "A,C" comma-letter keys (runtime CSV shape) count as a SET of
           selections, exactly like option-text lists do. */
        var splitLetters = function (list) {
          if (list.length === 1 && /^[A-D](,[A-D])+$/.test(String(list[0]).toUpperCase())) {
            return String(list[0]).toUpperCase().split(',');
          }
          return list;
        };
        var want = splitLetters((Array.isArray(q.answer) ? q.answer : parseList(q.answer)).map(norm)).map(norm);
        var got = splitLetters((Array.isArray(given) ? given : parseList(given)).map(norm)).map(norm);
